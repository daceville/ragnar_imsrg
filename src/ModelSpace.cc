...
//Electron_Orbit::~Electron_Orbit()
//{
// cout << "In Electron_Orbit destructor" << endl;
//}

//Electron_Orbit::Electron_Orbit()
//: n(-1), l(-1), ml(-1), ms(-1), occ(-1), cvq(-1), index(-1)
//{}

//Electron_Orbit::Electron_Orbit(int n, int l, int ml, float ms, double occ, int cvq, int index)
//: n(n), l(l), ml(ml), ms(ms), occ(occ), cvq(cvq), index(index)
//{}

//Electron_Orbit::Electron_Orbit(const Electron_Orbit& e_orb)
//: n(e_orb.n), l(e_orb.l), ml(e_orb.ml), ms(e_orb.ms), occ(e_orb.occ), cvq(e_orb.cvq), index(e_orb.index)
//{}
...
:  Emax(0), E2max(0), E3max(0), Lmax2(0), Lmax3(0), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0), norbits(0),
  hbar_omega(20), target_mass(16), isNuclear(true)
{
...
ModelSpace::ModelSpace(const ModelSpace& ms)
 :
   ...
   proton_orbits( ms.proton_orbits),neutron_orbits( ms.neutron_orbits),electron_orbits(ms.electron_orbits),
...
:  Emax(emax), E2max(2*emax), E3max(3*emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0), norbits(0), hbar_omega(20), target_mass(16), isNuclear(true)
{
...
: Emax(emax), E2max(2*emax), E3max(3*emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0),hbar_omega(20), isNuclear(true)
{
  Init(emax,reference,valence);
}

// Second most conventient (sp?) interface
ModelSpace::ModelSpace(int emax, string reference, string valence, bool setNuclear)
: Emax(emax), E2max(2*emax), E3max(3*emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0),hbar_omega(20), isNuclear(true)
{
  Init(emax,reference,valence,setNuclear);
}
...
ModelSpace::ModelSpace(int emax, string valence, bool setNuclear)
: Emax(emax), E2max(2*emax), E3max(3*emax), Lmax2(emax), Lmax3(emax), OneBodyJmax(0), TwoBodyJmax(0), ThreeBodyJmax(0),hbar_omega(20), isNuclear(setNuclear)
{
  auto itval = ValenceSpaces.find(valence);
  if ( itval != ValenceSpaces.end() ) // we've got a valence space
     Init(emax,itval->second[0],valence,setNuclear);
  else  // no valence space. we've got a single-reference.
     Init(emax,valence,valence,setNuclear);
}
...
//Same as above, but with SetNuclear
void ModelSpace::Init(int emax, vector<string> hole_list, vector<string> core_list, vector<string> valence_list, bool setNuclear)
{
   cout << "Creating a model space with Emax = " << Emax << "  and hole orbits [";
   for (auto& h : hole_list)  cout << h << " ";
   cout << "]   and core orbits [";
   ...
   map<index_t,double> hole_map;
   for (auto& h : String2Index(hole_list)) hole_map[h] = 1.0;
  Init(emax, hole_map, String2Index(core_list), String2Index(valence_list), setNuclear);
}
...
void ModelSpace::Init(int emax, map<index_t,double> hole_list, vector<index_t> core_list, vector<index_t> valence_list, bool setNuclear)
{
   ...
   { // build up Atomic orbits
         ...
         for (int l=0; l<n; l++)
	 {
      ...
      //if (reference =="H1"){ // This is garbage, but we can handle it later
         Zref=1;
         Aref=0;
      //}
   }
...
ModelSpace ModelSpace::operator=(const ModelSpace& ms)
{
   ...
   electron_orbits = ms.electron_orbits;
...
ModelSpace ModelSpace::operator=(ModelSpace&& ms)
{
   ...
   proton_orbits =  move(ms.proton_orbits);
   neutron_orbits =  move(ms.neutron_orbits);
   electron_orbits = move(ms.electron_orbits);
...
void ModelSpace::AddElectron_Orbit(Orbit orb)
{
  AddElectron_Orbit(orb.n, orb.l, orb.j2, orb.tz2, orb.occ, orb.cvq);
}

void ModelSpace::AddElectron_Orbit(int n, int l, int j2, int tz2, double occ, int cvq)
{
   index_t indy = Index1(n, l, j2, tz2);
   int ind = index3[indy];
   Orbits[ind] = Orbit(n, l, j2, tz2, occ, cvq, ind);
